...
import type { LocationUpload } from "@/lib/supabase/types";
import DeleteConfirmationDialog from "./DeleteConfirmationDialog";

interface LocationUploadHistoryProps {
  uploads: LocationUpload[];
  onRevert?: () => void;
}

export default function LocationUploadHistory({
  uploads,
  onRevert,
}: LocationUploadHistoryProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [revertingId, setRevertingId] = useState<string | null>(null);
  const [confirmRevert, setConfirmRevert] = useState<LocationUpload | null>(null);
  ...
  const handleRevert = async (upload: LocationUpload) => {
    setRevertingId(upload.id);
    try {
      const response = await fetch(
        `/api/admin/locations/uploads/${upload.id}/revert`,
        {
          method: "POST",
        }
      );
      ...
      alert(
        `Successfully reverted upload. ${result.deletedCount} locations were deleted.`
      );
      router.refresh();
      onRevert?.();
    } catch (error: any) {
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-sm text-navy-600 space-y-1">
                      ...
                      <p className="flex items-center gap-2">
                        <span className="text-navy-400">Locations:</span>
                        {upload.location_count}
                      </p>
                      <p className="flex items-center gap-2">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                ...
                <div className="text-sm text-amber-800">
                  ...
                  <p>
                    This will permanently delete all {confirmRevert.location_count} locations
